document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupEventListeners(userId) {
        ...
        document.getElementById('chatbot-form').addEventListener('submit', async e => { e.preventDefault(); const chatbotInput = document.getElementById('chatbot-input'); const userInput = chatbotInput.value.trim(); if (!userInput) return; addMessageToChat(userInput, 'user-message'); chatHistory.push({ role: "user", parts: [{ text: userInput }] }); chatbotInput.value = ''; addMessageToChat("...", 'bot-message'); try { const response = await fetch('/api/gemini', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt: userInput, history: chatHistory.slice(0, -1) }) }); if (!response.ok) throw new Error('Falha na API'); const data = await response.json(); document.querySelector('#chatbot-messages .bot-message:last-child').textContent = data.text; chatHistory.push({ role: "model", parts: [{ text: data.text }] }); } catch (error) { document.querySelector('#chatbot-messages .bot-message:last-child').textContent = `Erro: ${error.message}`; } await saveAllUserData(userId); });
    ...
    function initBotConnection() { const c = document.getElementById('bot-connection-area'); c.innerHTML = '<p>Tentando conectar ao servidor do bot... Isso pode levar até 1 minuto.</p>'; const es = new EventSource(`${BOT_BACKEND_URL}/events`); es.onmessage = function(ev) { const d = JSON.parse(ev.data); if (d.type === 'qr') { c.innerHTML = `<h3>Escaneie o QR Code</h3><img src="${d.data}" alt="QR Code" style="width:250px;height:250px;">`; } else if (d.type === 'status') { c.innerHTML = `<p>Status: <strong style="color:lightgreen;">${d.data}</strong></p>`; } }; es.onerror = function() { c.innerHTML = '<p style="color:red;">Não foi possível conectar. Verifique se o bot está ativo no Render e atualize a página.</p>'; es.close(); }; }
