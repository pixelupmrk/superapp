document.addEventListener('DOMContentLoaded', () => {
    ...
    function openLeadModal(leadId, userId) {
        ...
        leads[leadIndex].newMessages = 0; // Zera as notificações ao abrir
        saveAllUserData(userId); // Salva o estado zerado
            ...
            if (user && !document.body.hasAttribute('data-initialized')) {
                ...
                unsubscribeUserData = db.collection('userData').doc(user.uid).onSnapshot(doc => {
                    ...
                    if (doc.exists) {
                        ...
                        if (leads.length > oldLeadsCount) { // Apenas notifica se for um lead novo
                           updateNotificationBadge();
                        }
                        updateAllUI();
                    }
                }, err => {
                    console.log("Erro no listener do Firebase:", err);
                });
