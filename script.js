document.addEventListener('DOMContentLoaded', () => {
    // Conteúdo da Mentoria completo
    const mentoriaData = [{"moduleId":"MD01","title":"Módulo 1: Conectando com o Cliente Ideal","lessons":[{"lessonId":"L01.01","title":"Questionário para Definição de Persona","content":"Antes de qualquer estratégia, é essencial saber com quem você está falando..."}]}, {"moduleId":"MD02","title":"Módulo 2: O Algoritmo da Meta","lessons":[{"lessonId":"L02.01","title":"Como o Algoritmo Funciona","content":"O algoritmo da Meta analisa o comportamento dos usuários..."}]}, {"moduleId":"MD03","title":"Módulo 3: Cronograma de Postagens","lessons":[{"lessonId":"L03.01","title":"Melhores Horários e Dias","content":"O ideal é postar quando seu público está mais ativo..."}]}, {"moduleId":"MD04","title":"Módulo 4: Conteúdo que Conecta","lessons":[{"lessonId":"L04.01","title":"Estrutura de Vídeos que Engajam","content":"Um vídeo precisa seguir uma estrutura estratégica: Gancho, Valor e CTA..."}]}, {"moduleId":"MD05","title":"Módulo 5: Copywriting com ChatGPT","lessons":[{"lessonId":"L05.01","title":"Como Criar Textos Persuasivos","content":"O ChatGPT é uma ferramenta poderosa..."}]}, {"moduleId":"MD06","title":"Módulo 6: Implementação de CRM","lessons":[{"lessonId":"L06.01","title":"O que é CRM","content":"CRM é uma ferramenta que organiza o relacionamento..."}]}, {"moduleId":"MD07","title":"Módulo 7: Processo Comercial","lessons":[{"lessonId":"L07.01","title":"Como Montar um Pitch Comercial","content":"O pitch é a sua 'apresentação relâmpago'..."}]}, {"moduleId":"MD08","title":"Módulo 8: Conexão com a Audiência","lessons":[{"lessonId":"L08.01","title":"Gerando Conexão Real","content":"Pessoas se conectam com pessoas..."}]}];
    ...
    async function saveAllUserData(userId) {
        getMentoriaNotes();
        const settings = { userName: document.getElementById('setting-user-name').value || 'Usuário', theme: document.body.classList.contains('light-theme') ? 'light' : 'dark' };
    ...
    // --- EVENT LISTENERS (FINALMENTE CORRIGIDOS) ---
    function setupEventListeners(userId) {
        ...
        document.getElementById('chatbot-form').addEventListener('submit', async e => { e.preventDefault(); const chatbotInput = document.getElementById('chatbot-input'); const userInput = chatbotInput.value.trim(); if (!userInput) return; addMessageToChat(userInput, 'user-message'); chatHistory.push({ role: "user", parts: [{ text: userInput }] }); chatbotInput.value = ''; addMessageToChat("...", 'bot-message'); try { await saveAllUserData(userId); } catch (error) { console.error(error); } });
        ...
        document.getElementById('caixa-table').addEventListener('click', async e => { if (e.target.closest('.btn-delete-caixa')) { if (confirm('Tem certeza?')) { const itemIndex = e.target.dataset.index; caixa.splice(itemIndex, 1); await saveAllUserData(userId); updateAllUI(); } } });
    ...
    function initBotConnection() { const c = document.getElementById('bot-connection-area'); c.innerHTML = '<p>Tentando conectar ao servidor do bot... Isso pode levar até 1 minuto.</p>'; const es = new EventSource(`${BOT_BACKEND_URL}/events`); es.onmessage = ev => { const d = JSON.parse(ev.data); if (d.type === 'qr') c.innerHTML = `<h3>Escaneie o QR Code</h3><img src="${d.data}" alt="QR Code" style="width:250px;height:250px;">`; else if (d.type === 'status') c.innerHTML = `<p>Status: <strong style="color:lightgreen;">${d.data}</strong></p>`; }; es.onerror = () => { c.innerHTML = '<p style="color:red;">Não foi possível conectar. Verifique se o bot está ativo no Render e atualize a página.</p>'; es.close(); }; }
    ...
    function renderCaixaTable() { const t = document.querySelector('#caixa-table tbody'); if (t) t.innerHTML = caixa.map((m, i) => `<tr><td>${m.data}</td><td>${m.descricao}</td><td>${m.tipo === 'entrada' ? 'R$ ' + parseFloat(m.valor).toFixed(2) : ''}</td><td>${m.tipo === 'saida' ? 'R$ ' + parseFloat(m.valor).toFixed(2) : ''}</td><td><button class="btn-table-action btn-delete-item btn-delete-caixa" data-index="${i}">Excluir</button></td></tr>`).join(''); }
