document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderCaixaTable() { 
        const t = document.querySelector('#caixa-table tbody'); 
        if (t) { 
            t.innerHTML = caixa.map(m => `<tr><td>${m.data}</td><td>${m.descricao}</td><td class="entrada">${m.tipo==='entrada'?'R$ '+m.valor.toFixed(2):''}</td><td class="saida">${m.tipo==='saida'?'R$ '+m.valor.toFixed(2):''}</td><td>${m.observacoes}</td></tr>`).join(''); 
        } 
    }
    ...
    function addMessageToChat(msg, type) { 
        ...
        if(c) { 
            c.innerHTML += `<div class="message-wrapper"><div class="${type}">${msg}</div></div>`; 
