document.addEventListener('DOMContentLoaded', () => {
    // --- DADOS COMPLETOS DA MENTORIA (sem alterações) ---
    const mentoriaData = [
    ...
    let statusChart;
    let db;
    let unsubscribeChatListener = null; // Para podermos desligar o ouvinte ao fazer logout
    ...
    async function loadAllUserData(userId) {
        try {
            ...
            // --- ALTERAÇÃO 1: OUVINTE DE CHAT EM TEMPO REAL ---
            // Removemos o carregamento antigo do chatHistory e ativamos um ouvinte
            // que atualiza a tela sempre que uma nova mensagem chega.
            if (unsubscribeChatListener) unsubscribeChatListener(); // Cancela ouvinte anterior se houver
            
            unsubscribeChatListener = db.collection('userData').doc(userId).collection('chatHistory').orderBy('timestamp')
              .onSnapshot(snapshot => {
                  chatHistory = []; // Limpa o histórico local para reconstruir
                  const messagesContainer = document.getElementById('chatbot-messages');
                  if (!messagesContainer) return;
                  messagesContainer.innerHTML = ''; // Limpa a tela do chat
                  ...
                  snapshot.forEach(doc => {
                      ...
                      chatHistory.push(message); // Atualiza o histórico local
                      addMessageToChat(message.parts[0].text, message.role === 'user' ? 'user-message' : 'bot-message');
                  });
              }, error => {
                  console.error("Erro no ouvinte do chat: ", error);
              });
            // --- FIM DA ALTERAÇÃO 1 ---

            renderMentoria();
            updateAllUI(); // Atualiza o resto da UI que não é em tempo real
    ...
    async function saveUserData(userId) {
        try {
            const dataToSave = {
                leads, caixa, estoque, // O chatHistory não é mais salvo daqui, o backend cuida disso
    ...
    function updateAllUI() {
        ...
        updateCaixa();
        renderEstoqueTable();
        // A renderização do chat agora é feita pelo ouvinte em tempo real
    }
    ...
    function setupEventListeners(userId) {
        ...
        if (menuToggle && appContainer) {
            menuToggle.addEventListener('click', () => appContainer.classList.toggle('sidebar-visible'));
        }
        document.querySelectorAll('.sidebar-nav .nav-item').forEach(item => {
            item.addEventListener('click', e => {
                if (window.innerWidth <= 768) appContainer.classList.remove('sidebar-visible');
        ...
        // --- ALTERAÇÃO 2: NOVO EVENTO DE SUBMISSÃO DO CHATBOT ---
        // Substituímos o 'fetch' antigo por uma chamada para nossa nova Cloud Function 'startAiChat'.
        // O app não espera mais pela resposta, ele apenas envia a pergunta e o ouvinte em tempo real cuida do resto.
        document.getElementById('chatbot-form')?.addEventListener('submit', async e => {
            ...
            try {
                addMessageToChat("Enviando...", 'user-message bot-thinking'); // Feedback visual
                const startAiChatFunction = firebase.functions().httpsCallable('startAiChat');
                await startAiChatFunction({ prompt: messageToSend });
                document.querySelector('.bot-thinking')?.remove(); // Remove o feedback visual
            } catch (error) {
                console.error("Erro ao chamar a Cloud Function:", error);
                document.querySelector('.bot-thinking')?.remove();
                addMessageToChat(`Erro ao enviar: ${error.message}`, 'bot-message');
            }
        });
        // --- FIM DA ALTERAÇÃO 2 ---
    ...
    // ... (Todas as suas funções de renderização e modais permanecem exatamente iguais) ...
