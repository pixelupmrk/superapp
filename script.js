document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('[DEBUG] DOM carregado. Iniciando script...');
        ...
        document.querySelectorAll('.sidebar-nav .nav-item').forEach(item => {
            item.addEventListener('click', e => {
                try {
                    ...
                    console.error('[DEBUG] ERRO no listener da sidebar:', error);
                }
            });
        });
    ...
    function renderMentoria(mentoriaData) {
        ...
        if (!menu || !content || !mentoriaData || mentoriaData.length === 0) return;
        ...
        content.innerHTML = mentoriaData.map(mod => {
            ...
            return `<div class="mentoria-module-content" id="${mod.moduleId}">${lessonsHtml}<div class="anotacoes-aluno"><textarea class="mentoria-notas" id="notas-${mod.moduleId}" rows="8" placeholder="Suas anotações..."></textarea></div></div>`;
        }).join('');
