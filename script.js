document.addEventListener('DOMContentLoaded', () => {
    ...
    function applySettings(settings = {}) {
        ...
        if (settingUserNameInput) settingUserNameInput.value = userName;
    }
    ...
    function setupEventListeners(userId) {
        ...
        document.getElementById('leads-table')?.addEventListener('click', e => { e.preventDefault(); const leadRow = e.target.closest('tr[data-id]'); if (leadRow) { openLeadModal(parseInt(leadRow.dataset.id), userId); } });
        ...
        document.querySelectorAll('.close-modal').forEach(btn => { btn.addEventListener('click', () => { const modal = document.getElementById(btn.dataset.target); if (modal) modal.style.display = 'none'; if (unsubscribeChat) unsubscribeChat(); }); });
        ...
        document.getElementById('caixa-table')?.addEventListener('click', async e => { const deleteBtn = e.target.closest('.btn-delete-caixa'); if (deleteBtn) { if (confirm('Tem certeza?')) { caixa = caixa.filter(item => item.id !== deleteBtn.dataset.id); await saveAllUserData(userId); updateAllUI(); } } });
        document.querySelectorAll('.finance-tab').forEach(tab => { tab.addEventListener('click', e => { e.preventDefault(); document.querySelectorAll('.finance-tab, .finance-content').forEach(el => el.classList.remove('active')); e.target.classList.add('active'); const contentTab = document.getElementById(e.target.dataset.tab + '-tab-content'); if(contentTab) contentTab.classList.add('active'); }); });
        ...
        document.getElementById('estoque-table')?.addEventListener('click', async e => { if (e.target.closest('.btn-custo')) { openCustosModal(e.target.closest('tr').dataset.id); } if (e.target.closest('.btn-delete-estoque')) { if (confirm('Tem certeza?')) { estoque = estoque.filter(p => p.id !== e.target.closest('tr').dataset.id); await saveAllUserData(userId); updateAllUI(); } } });
    ...
    function renderKanbanCards() { const lists = document.querySelectorAll('.kanban-cards-list'); if (lists) lists.forEach(l => l.innerHTML = ''); leads.forEach(lead => { const c = document.querySelector(`.kanban-column[data-status="${lead.status}"] .kanban-cards-list`); if (c) c.innerHTML += `<div class="kanban-card" draggable="true" data-id="${lead.id}"><strong>${lead.nome}</strong><p>${lead.whatsapp}</p></div>`; }); }
