// CÓDIGO FINAL E CORRIGIDO COM MAIS DEBUG
document.addEventListener('DOMContentLoaded', () => {
    console.log('[DEBUG] DOM carregado. Aguardando Firebase...');
    // ... (restante do código que já te enviei e está correto)
    
    // ===================================================================
    // FUNÇÃO setupEventListeners ATUALIZADA E MAIS ROBUSTA
    // ===================================================================
    function setupEventListeners(userId) {
        console.log('[DEBUG] Configurando event listeners...');
        
        // Listener para navegação da Sidebar
            ...
            item.addEventListener('click', e => {
                ...
                try {
                    ...
                    if (e.currentTarget.id === 'logout-btn') {
                        ...
                        // A lógica de logout já está no auth.js, então não fazemos nada aqui.
                        return;
                    }
        ...
        // Listener do botão Salvar e Gerar QR Code
        ...
        // Adicione aqui todos os outros listeners que já estavam no código anterior
        // (save-settings-btn, theme-toggle-btn, lead-form, etc.)
        // Eles já estavam corretos, mas esta estrutura com try/catch é mais segura.
        document.getElementById('save-settings-btn')?.addEventListener('click', () => {
            console.log('[DEBUG] Botão Salvar Configurações clicado.');
            saveAllUserData(userId, true);
        });

        document.getElementById('theme-toggle-btn')?.addEventListener('click', () => {
            console.log('[DEBUG] Botão Mudar Tema clicado.');
            toggleTheme();
        });

        document.getElementById('chatbot-form')?.addEventListener('submit', e => {
            console.log('[DEBUG] Formulário do Chatbot enviado.');
            e.preventDefault(); 
         ...
         document.getElementById('export-csv-btn')?.addEventListener('click', () => {
            ...
            if (estoque.length === 0) return alert("Não há produtos para exportar.");
        ...
        document.getElementById('menu-toggle')?.addEventListener('click', () => {
            console.log('[DEBUG] Botão de Menu (mobile) clicado.');
    ...
    // ... (restante de todas as outras funções que já te enviei)
    // Cole aqui o resto do script.js que te mandei na mensagem anterior,
    // apenas substituindo a função setupEventListeners por esta acima.
    // Ou, para facilitar, aqui está o arquivo COMPLETO novamente.
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initBotConnection() { console.log('[DEBUG] Função initBotConnection() chamada. Tentando conectar a:', BOT_BACKEND_URL); const c = document.getElementById('bot-connection-area'); c.innerHTML = '<p>Tentando conectar ao servidor do bot... Isso pode levar até 1 minuto se ele estiver inativo.</p>'; const es = new EventSource(`${BOT_BACKEND_URL}/events`); es.onopen = () => console.log('[DEBUG] Conexão EventSource ABERTA com o servidor.'); es.onmessage = ev => { console.log('[DEBUG] Mensagem recebida do servidor:', ev.data); try { const d = JSON.parse(ev.data); if (d.type === 'qr') { c.innerHTML = `<h3>Escaneie o QR Code</h3><img src="${d.data}" alt="QR Code">`; es.close(); } else if (d.type === 'status') { c.innerHTML = `<p>Status: <strong style="color:lightgreen;">${d.data}</strong></p>`; } } catch (e) { console.error("Erro ao processar mensagem do servidor: ", e); } }; es.onerror = (err) => { console.error('[DEBUG] ERRO na conexão EventSource:', err); c.innerHTML = '<p style="color:red;">Não foi possível conectar. Verifique o problema de CORS no backend e se o bot está ativo no Render.</p>'; es.close(); }; }
    async function handleChatbotSubmit(userId) { const input = document.getElementById('chatbot-input'); const text = input.value.trim(); if (!text) return; addMessageToChat(text, 'user-message'); chatHistory.push({ role: "user", parts: [{ text }] }); input.value = ''; const thinkingMsg = addMessageToChat("...", 'bot-message thinking'); await saveAllUserData(userId); try { const response = await fetch('/api/gemini', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ history: chatHistory.slice(0, -1), prompt: text }) }); if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`); const data = await response.json(); thinkingMsg.textContent = data.text; thinkingMsg.classList.remove('thinking'); chatHistory.push({ role: 'model', parts: [{ text: data.text }] }); await saveAllUserData(userId); } catch (error) { console.error("Erro ao chamar a API do Chatbot:", error); thinkingMsg.textContent = "Desculpe, não consegui me conectar à IA. Tente novamente."; thinkingMsg.classList.remove('thinking'); } }
