...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadAllUserData(userId) {
        ...
        try {
            ...
            renderChatHistory();
            updateAllUI();
            console.log('[DEBUG] Dados carregados e UI atualizada.');
        } catch (error) {
        ...
        if(saveBtn) {
            saveBtn.addEventListener('click', async () => {
                ...
                try {
                    ...
                    alert('Instruções salvas! Iniciando conexão...');
                    initBotConnection();
                } catch (error) {
    ...
    function initBotConnection() {
        ...
        es.onmessage = ev => {
            ...
            try {
                ...
                if (d.type === 'qr') {
                    ...
                    console.log('[DEBUG] QR Code renderizado.');
                    es.close();
        ...
        es.onerror = (err) => {
            ...
            c.innerHTML = '<p style="color:red;">Não foi possível conectar. Verifique se o bot está ativo no Render e tente novamente.</p>';
            es.close();
        };
    }
    ...
    function checkTheme() {
        ...
        if (theme === 'light') {
            ...
            if (btn) btn.textContent = 'Mudar para Tema Escuro';
        } else {
            document.body.classList.remove('light-theme');
            if (btn) btn.textContent = 'Mudar para Tema Claro';
        }
    }
    ...
    function addMessageToChat(msg, type) { const container = document.getElementById('chatbot-messages'); if (!container) return null; const msgDiv = document.createElement('div'); msgDiv.classList.add(type); msgDiv.textContent = msg; container.appendChild(msgDiv); container.scrollTop = container.scrollHeight; return msgDiv; }
