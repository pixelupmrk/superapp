document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupEventListeners(userId) {
        ...
        // **CORREÇÃO**: Adiciona o event listener para as abas do Financeiro
        ...
        document.getElementById('lead-form')?.addEventListener('submit', async (e) => {
            ...
            const newLead = { 
                ...
                origem: document.getElementById('lead-origin').value, // **CORREÇÃO**: Adiciona o campo origem
        ...
        if (kanbanBoard) {
            ...
            kanbanBoard.addEventListener('drop', async (e) => {
                ...
                if (column && draggedItem) {
                    const leadId = parseInt(draggedItem.dataset.id, 10); // **CORREÇÃO**: Garante que o ID é um número
            ...
            // **CORREÇÃO**: Event listener para abrir o modal a partir do Kanban
            kanbanBoard.addEventListener('click', e => { 
                if (e.target.tagName === 'A') { e.stopPropagation(); return; } 
                const card = e.target.closest('.kanban-card'); 
                if (card) {
                    openEditModal(parseInt(card.dataset.id, 10)); // **CORREÇÃO**: Garante que o ID é um número
                }
            });
        }
        
        // **CORREÇÃO**: Event listener para abrir o modal a partir da Lista
        document.getElementById('leads-table')?.addEventListener('click', e => { 
            ...
            if (editButton) {
                const leadId = parseInt(editButton.closest('tr').dataset.id, 10); // **CORREÇÃO**: Garante que o ID é um número
                openEditModal(leadId);
            }
            if (deleteButton) {
                if (confirm('Tem certeza que deseja excluir este lead?')) { 
                    const leadId = parseInt(deleteButton.closest('tr').dataset.id, 10); // **CORREÇÃO**: Garante que o ID é um número
    ...
    // **CORREÇÃO**: Função de renderizar mentoria com placeholders dinâmicos e layout corrigido
    function renderMentoria() {
        ...
        content.innerHTML = mentoriaData.map((mod, i) => { 
            ...
            // **CORREÇÃO**: Substitui quebras de linha por <br> para formatação correta
        ...
        document.querySelectorAll('.sales-accelerator-menu-item').forEach(item => { 
            item.addEventListener('click', e => { 
                ...
                document.getElementById(clickedItem.dataset.moduleId).classList.add('active'); 
            }); 
        });

        // Garante que o primeiro item esteja ativo ao carregar
        const firstMenuItem = document.querySelector('.sales-accelerator-menu-item');
        if (firstMenuItem) {
            firstMenuItem.classList.add('active');
            const firstModule = document.getElementById(firstMenuItem.dataset.moduleId);
            if (firstModule) {
